<?php
// planning-handler.php (Final Corrected Version)
...
try {
    ...
    switch ($action) {
        case 'get_initial_data':
            getInitialData($conn);
            break;
        case 'save_mission': // For creating AND validating a new single mission
            saveMission($conn, $currentUser['user_id'], $input);
            break;
        case 'save_and_validate_multiple': // For "Activate All" button
            saveAndValidateMultiple($conn, $currentUser['user_id'], $input);
            break;
        case 'update_mission': // For updating an existing mission
...
function saveMission($conn, $creator_id, $data) {
    ...
    if (empty($data['assigned_user_ids'])) respondWithError('Veuillez assigner au moins un ouvrier.');
    ...
    if (empty($dates)) respondWithError('La date de la mission est obligatoire.');

    $conn->beginTransaction();
    try {
        $mission_group_id = $conn->query("SELECT NEWID()")->fetchColumn();
        saveMissionData($conn, $creator_id, $data, $dates, $mission_group_id);
        $conn->commit();
...
// NEW function for "Activate All" button
function saveAndValidateMultiple($conn, $creator_id, $data) {
    if (empty($data['missions']) || !is_array($data['missions'])) {
        respondWithError('Aucune mission à enregistrer.');
    }
    ...
    try {
        foreach($data['missions'] as $missionData) {
            $mission_group_id = $conn->query("SELECT NEWID()")->fetchColumn();
            $dates = [$missionData['assignment_date']]; // Each temp mission is for a single date
            saveMissionData($conn, $creator_id, $missionData, $dates, $mission_group_id);
        }
        $conn->commit();
        respondWithSuccess('Missions temporaires enregistrées et validées.');
    } catch (Exception $e) {
        $conn->rollBack();
        respondWithError('Erreur lors de l\'enregistrement des missions: ' . $e->getMessage(), 500);
    }
}


// Refactored helper function for saving mission data
function saveMissionData($conn, $creator_id, $data, $dates, $mission_group_id) {
    ...
    if (!empty($assigned_asset_ids) && !empty($dates)) {
        ...
        if ($conflict = $stmt_check->fetch(PDO::FETCH_ASSOC)) {
            throw new Exception("Conflit: L'actif '{$conflict['asset_name']}' est déjà réservé le " . date('d/m/Y', strtotime($conflict['booking_date'])) . ".");
        }
    }
    ...
    foreach ($dates as $mission_date) {
        foreach ($assigned_users as $user_id) {
            $stmt_insert->execute([$user_id, $creator_id, $mission_date, $data['start_time'] ?: null, $data['end_time'] ?: null, $data['shift_type'], $data['mission_text'], $data['comments'] ?? null, $data['color'], $data['location'] ?: null, $mission_group_id]);
        }
...
function updateMission($conn, $data) {
    ...
    try {
        ...
        if (empty($mission_dates)) { $conn->rollBack(); respondWithError("Mission non trouvée."); }
        ...
        $stmt_update->execute([$data['mission_text'], $data['comments'] ?? null, $data['start_time'] ?: null, $data['end_time'] ?: null, $data['location'] ?: null, $data['shift_type'], $data['color'], $mission_group_id]);
        ...
        $conn->rollBack();
        respondWithError('Erreur lors de la mise à jour : ' . $e->getMessage(), 500);
    }
}
...
function deleteMissionGroup($conn, $data) {
    ...
    try {
        ...
        $conn->rollBack();
        respondWithError('Erreur lors de la suppression : ' . $e->getMessage(), 500);
    }
}

function assignWorkerToMission($conn, $creator_id, $data) {
    $worker_id = $data['worker_id']; $mission_group_id = $data['mission_group_id'];
...
function removeWorkerFromMission($conn, $data) {
    ...
    try {
        ...
        $conn->commit();
        respondWithSuccess('Ouvrier retiré.');
    } catch (Exception $e) {
        $conn->rollBack();
        respondWithError('Erreur lors du retrait : ' . $e->getMessage(), 500);
    }
}
...
function toggleMissionValidation($conn, $data) {
    ...
    $stmt_orig->execute([$mission_group_id]);
    $new_status = $stmt_orig->fetchColumn() ? 0 : 1;
...
function validateAllForWeek($conn, $data) {
    ...
    respondWithSuccess('Planifications existantes mises à jour.');
}
