<?php
// planning-handler.php (Corrected with scope logic)
...
try {
    ...
    switch ($action) {
        ...
        case 'delete_mission_group':
            ...
            break;
        case 'assign_worker_to_mission': // Route based on scope
            if (($input['scope'] ?? 'day') === 'series') {
                assignWorkerToMissionSeries($conn, $currentUser['user_id'], $input);
            } else {
                assignWorkerToMissionDay($conn, $currentUser['user_id'], $input);
            }
            break;
        case 'remove_worker_from_mission': // Route based on scope
             if (($input['scope'] ?? 'day') === 'series') {
                removeWorkerFromMissionSeries($conn, $input);
            } else {
                removeWorkerFromMissionDay($conn, $input);
            }
            break;
...
function getInitialData($conn) {
    ...
    $stmt_missions = $conn->prepare("
        SELECT
            pa.assignment_id as mission_id,
            pa.mission_group_id,
            ...
            STRING_AGG(CAST(u.user_id AS VARCHAR(MAX)), ',') WITHIN GROUP (ORDER BY u.nom, u.prenom) as assigned_user_ids,
        ...
        GROUP BY
            pa.assignment_id, pa.mission_group_id, pa.assignment_date, pa.mission_text, pa.comments, pa.location, 
            pa.start_time, pa.end_time, pa.shift_type, pa.color, pa.is_validated
...
function getWorkerStatusForDate($conn, $date) {
    ...
    foreach ($all_users as $user_id) {
        ...
        if (in_array($user_id, $assigned_users)) {
            $status = 'assigned';
        } elseif (in_array($user_id, $on_leave_users)) {
            $status = 'on_leave';
        }
...
function assignWorkerToMissionDay($conn, $creator_id, $data) {
    $worker_id = $data['worker_id'];
    $mission_id = $data['mission_id']; // This is the assignment_id of the specific day
    if (!$worker_id || !$mission_id) respondWithError('Données manquantes.');
    ...
    if (!$mission_template) respondWithError('Mission cible non trouvée.');

    $stmt_check = $conn->prepare("SELECT COUNT(*) FROM Planning_Assignments WHERE mission_group_id = ? AND assigned_user_id = ? AND assignment_date = ?");
    $stmt_check->execute([$mission_template['mission_group_id'], $worker_id, $mission_template['assignment_date']]);
    if ($stmt_check->fetchColumn() > 0) {
        respondWithSuccess('Ouvrier déjà assigné à cette mission pour cette date.');
        return;
    }
    ...
    $stmt_insert->execute([$worker_id, $creator_id, $mission_template['assignment_date'], $mission_template['start_time'], $mission_template['end_time'], $mission_template['shift_type'], $mission_template['mission_text'], $mission_template['comments'], $mission_template['color'], $mission_template['location'], $mission_template['is_validated'], $mission_template['mission_group_id']]);
    
    respondWithSuccess('Ouvrier assigné pour cette date.');
}

function assignWorkerToMissionSeries($conn, $creator_id, $data) {
    $worker_id = $data['worker_id'];
    $mission_id = $data['mission_id'];
    if (!$worker_id || !$mission_id) respondWithError('Données manquantes.');
    ...
    if ($stmt_check->fetchColumn() > 0) {
        $conn->rollBack();
        respondWithSuccess('Ouvrier déjà assigné à cette série de missions.');
        return;
    }
...
function removeWorkerFromMissionDay($conn, $data) {
    $worker_id = $data['worker_id'];
    $mission_id = $data['mission_id']; // assignment_id
    if (!$worker_id || !$mission_id) respondWithError('Données manquantes.');
    ...
    respondWithSuccess('Ouvrier retiré de la mission pour cette date.');
}

function removeWorkerFromMissionSeries($conn, $data) {
    $worker_id = $data['worker_id'];
    $mission_id = $data['mission_id'];
    if (!$worker_id || !$mission_id) respondWithError('Données manquantes.');
    ...
    if (!$mission_group_id) {
        respondWithError('Groupe de mission non trouvé.');
    }
    ...
    respondWithSuccess('Ouvrier retiré de toute la série de missions.');
}
