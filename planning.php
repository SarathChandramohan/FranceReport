...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let state = { staff: [], missions: [], currentWeekStart: getMonday(new Date()), draggedWorker: null };
    ...
    function renderWorkerList() {
        ...
        state.staff.forEach(worker => {
            ...
            $workerList.append(`<div class="worker-item ${isAssigned ? 'assigned' : ''}" draggable="${!isAssigned}" data-worker-id="${worker.user_id}" data-worker-name="${worker.prenom} ${worker.nom}">
                    ...
                    ${isAssigned ? '<div class="assigned-info small text-danger">Déjà affecté</div>' : ''}
                </div>`);
        });
    }
    ...
    $workerList.on('dragstart', '.worker-item:not(.assigned)', (e) => { state.draggedWorker = { id: $(e.currentTarget).data('worker-id'), name: $(e.currentTarget).data('worker-name') }; });
    ...
    $('#missionForm').on('submit', async function(e) {
        ...
        if (!formData.mission_id) { // Only for new missions
