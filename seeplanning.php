...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderPlanning(missions) {
        ...
        if (!missions || missions.length === 0) {
            planningList.innerHTML = `<li class="placeholder"><i class="far fa-calendar-check"></i><p>Aucune mission planifiée.</p></li>`;
            return;
        }
    ...
    function showLoadingState() { planningList.innerHTML = `<li class="placeholder"><div class="spinner-border text-primary" role="status"><span class="sr-only">Loading...</span></div></li>`; }
    function showErrorState(message) { planningList.innerHTML = `<li class="placeholder"><i class="fas fa-exclamation-triangle text-danger"></i><p class="text-danger">${escapeHtml(message) || 'Une erreur est survenue.'}</p></li>`; }
