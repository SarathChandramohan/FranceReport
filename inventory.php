    ...
    <div id="booking" class="tab-content">
        ...
        <div id="history-content" class="booking-content-pane">
            <div class="card">
                ...
                <div class="booking-filters">
                    <input type="text" id="historyFilterDate" class="form-control" placeholder="Filtrer par date et heure..." style="max-width: 200px;">
                    ...
                    <input type="text" id="historyFilterItem" class="form-control" placeholder="Filtrer par item..." style="max-width: 250px;">
                </div>
                <div class="table-responsive">
                    <table class="table table-striped table-hover">
                        <thead class="thead-dark">
                            <tr><th>Picked-up Date</th><th>Submitted Date</th><th>Item</th><th>User</th><th>Mission</th></tr>
                        </thead>
...
function initializeBookingTabFilters() {
    const futureDateConfig = { locale: "fr", dateFormat: "Y-m-d", allowInput: true };
    flatpickr("#individualFilterDate", futureDateConfig);
    flatpickr("#missionFilterDate", futureDateConfig);

    const historyDateConfig = { locale: "fr", dateFormat: "Y-m-d H:i", enableTime: true, allowInput: true };
    flatpickr("#historyFilterDate", historyDateConfig);
}
...
function renderUsageHistoryTable() {
    ...
    const filtered = usageHistory.filter(h => 
        (!dateFilter || h.pickedup_date.includes(dateFilter) || (h.submitted_date && h.submitted_date.includes(dateFilter))) && 
    ...
    filtered.forEach(h => {
        const row = tableBody.insertRow();
        const pickedUp = h.pickedup_date ? new Date(h.pickedup_date).toLocaleString('fr-FR') : 'N/A';
        const submitted = h.submitted_date ? new Date(h.submitted_date).toLocaleString('fr-FR') : 'N/A';
        row.innerHTML = `<td>${pickedUp}</td><td>${submitted}</td><td>${h.asset_name || '(Supprimé)'}</td><td>${(h.prenom && h.nom) ? `${h.prenom} ${h.nom}` : 'N/A'}</td><td>${h.mission || 'N/A'}</td>`;
    });
}
